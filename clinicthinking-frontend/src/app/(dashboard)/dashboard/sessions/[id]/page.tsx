    ...
    <div className="p-8 max-w-3xl mx-auto">
      ...
      {session.status === 'in_progress' && (
        <div className="bento-glass-card rounded-none p-6 text-center">
          ...
          <p className="text-sm text-gray-500 mb-4">Lanjutkan sesi di halaman whiteboard.</p>
          <button
            onClick={() => router.push(`/session/${id}`)}
            className="bg-[#2c9ca0] text-white font-serif font-semibold px-6 py-2.5 rounded-none hover:bg-[#217578] transition-all text-sm inline-flex items-center gap-2"
          >
            <Clock size={14} /> Lanjutkan Sesi Whiteboard
          </button>
        </div>
      )}
      ...
      {session.status === 'submitted' && analysis && (
        <div className="space-y-4">
          ...
          <div className="flex flex-col sm:flex-row gap-3">
            <button
              onClick={() => router.push(`/session/${id}?mode=view`)}
            ...
            >
              <Brain size={15} /> Lihat Sesi & Node Whiteboard
            </button>
            <button
              onClick={() => router.push('/dashboard/sessions/new')}
              className="flex-1 flex items-center justify-center gap-2 bg-white/60 border border-[#2c9ca0]/20 text-gray-700 font-serif font-semibold py-3.5 rounded-none hover:bg-white/80 transition-all text-sm"
            >
